...
int main() {
  ...
  h.onMessage([&reference_speed,&lane_num,&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            for (int i = 0; i < sensor_fusion.size(); i++) {
              ...
              if (d>=0 && d<=12) {
                ...
                if (d<(2+4*lane_num+2) && d>(2+4*lane_num-2)) {
                  ...
                  if ((car_s - check_car_s < 20) && ((check_car_s - car_s) < 30)) {
                    car_left = true;
                  }
                } else if (d<(2+(lane_num+1)+2) && d>(2+(lane_num+1)-2)) {
                  if ((car_s - check_car_s < 20) && ((check_car_s - car_s) < 30)) {
            ...
            if (prev_size < 2) {
              ...
              reference_yaw = atan2(reference_y-reference_prev_y,reference_x-reference_prev_x);
