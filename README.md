...
*  Code can be compiled without errors with cmake and make.

### Valid Trajectories
##### 1.  The car is able to drive at least 4.32 miles without incident.

* The car can drive at least 10 mills without any collisions.

##### 2. The car drives according to the speed limit.

* Most time the car is driving at around 49.5 mph and never faster than the speed limit.
...
* During the simulation, the car doesn't collide other cars.

##### 5. The car stays in its lane, except for the time between changing lanes.

* The car is able to stay in its lane except changing lanes.

##### 6. The car is able to change lanes

* The car can change its lane when the car ahead is slower than its speed.
...
According to the result from the prediction module, this part is used to decide the car's behavior. For the car in this simulation, three states was used: `KL,LCR,LCL`. If there is a car blocking my current lane and the left lane is open, lane change to left `LCL` would be implemented. While, if the left lane is also blocked, lane change to right `LCR` would be implemented when right lane is open. Otherwise, if no passing lane is available, current car would like to keep current lane `KL` and slow down to maintain the buffer distance. Last but not least, the central lane is always the preferred option rather than the passing lane or the low-speed lane.
